...
const parse = ( Node, str, options ) => {
  ...
  const parsedNodes = lines.reduce(( nodes, line ) => {
    ...
    current[ metaSymbol ] = { indent, prev }
  ...
  if( parsedNodes.length === 0 )
    throw Error( 'Cannot deserialize empty file' )

  if( parsedNodes[ 0 ][ metaSymbol ].indent !== 0  )
    throw Error( 'Bad nesting' )

  const rootNodeIndices = parsedNodes.reduce(( indices, node, index ) => {
    if( node[ metaSymbol ].indent === 0 ) {
      indices.push( index )
    }
  ...
  if( deserializeMultiple ) {
    ...
    throw Error( 'Multiple roots and deserializeMultiple=false' )
  }
}
...
const parsedNodesToTree = parsedNodes => {
  ...
  parsedNodes.forEach( current => {
    ...
    let parent = current[ metaSymbol ].prev

    while( parent && parent[ metaSymbol ].indent >= indent ) {
      parent = parent[ metaSymbol ].prev
    }
