...
const existing = require( '../src/api' )
...
describe( 'promises', () => {
	...
	it( 'can promisify functions', () => {
		return mkdir( '/test' )
			.then( () => writeFile( '/test/text.txt', 'ABC', 'utf8' ) )
			.then( () => stat( '/test' ) )
			.then( stats => {
				stats.isDirectory().should.be.eql( true )
			})
			.then( () => stat( '/test/text.txt' ) )
			.then( stats => {
				stats.isFile().should.be.eql( true )
			})
	})
})
describe( 'plugins', () => {
	it( 'new plugin', () => {
		...
		const fs = Fs( {}, { api, asyncNames } )

		fs.writeFileSync( '/test.txt', 'abc' )
		fs.writeFileSync( '/empty.txt', '' )

		fs.isEmptyFileSync( '/test.txt' ).should.be.eql( false )
		fs.isEmptyFileSync( '/empty.txt' ).should.be.eql( true )
	...
	it( 'extend plugins', () => {
		...
		logged.should.be.deepEqual( [ fs, '/test.txt', 'abc' ] )
	})
})